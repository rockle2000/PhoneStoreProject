            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header bg-primary">
                        <h3 class="card-title">Danh sách thể loại bài viết</h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ url('edit-newscategory/'.$item->MaTheLoai) }}" class="btn btn-primary"><i class="fas fa-edit"></i> Edit</a>
                                        <span>
                                            @if ($item->TrangThai)
                                            <a href="#" onclick="return ConfirmDelete('{{ $item->MaTheLoai }}',this)" class="btn btn-danger"><i class="fas fa-trash-alt"></i> Delete</a>
                                            @elseif(!$item->TrangThai)
                                            <a href="#" onclick="return ConfirmActive('{{ $item->MaTheLoai }}',this)" class="btn btn-success"><i class="fas fa-plus"></i> Active</a>
                                            @endif
                                        </span>
                                    </td>
                                </tr>
...
<script type="text/javascript">
    ...
    function ConfirmActive(id, ctl) {
        Swal.fire({
            title: 'Bạn chắc chắn muốn active thể loại này?'
            , text: "Thể loại này sẽ được active"
            , icon: 'info'
    ...
    function ConfirmDelete(id, ctl) {
        //Disable sản phẩm
        Swal.fire({
            title: 'Bạn chắc chắn muốn ẩn thể loại này?'
            , text: "Thể loại tin tức này sẽ bị ẩn"
            , icon: 'warning'
            , showCancelButton: true
            , confirmButtonColor: '#3085d6'
            , cancelButtonColor: '#d33'
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    , url: '/PhoneStore/delete-newscategory/' + id
                    , success: function(result) {
                        if (result.status == 'success') {
